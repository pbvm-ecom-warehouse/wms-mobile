...
import { ArrowLeft, Box, CheckCircle2, Lock, RefreshCw, Sparkles, X } from 'lucide-react-native';
  ...
  const cells: StorageCellInfo[] = useMemo(() => {
    if (realCells && realCells.length > 0) {
      return realCells.map((rc) => {
        const isSuggested = suggestedCellCodes.some(
          (sc) => sc.includes(rc.code) || rc.code.includes(sc),
        );
    ...
    for (let l = levelsCount; l >= 1; l--) {
      for (let b = 1; b <= baysCount; b++) {
        ...
        const isSuggested =
          suggestedCellCodes.some((sc) => sc.includes(code) || code.includes(sc)) ||
          (l === 1 && b === 1);
        ...
        <View className="bg-[#f8fafc] rounded-t-3xl border-t border-[#e4e5e9] max-h-[92%] pb-6">
          ...
          <View className="px-4 py-2.5 bg-white border-b border-[#e2e8f0] flex-row justify-between items-center">
            {onBackToMap ? (
              <TouchableOpacity
                onPress={onBackToMap}
                className="bg-[#eff6ff] border border-[#bfdbfe] px-3 py-1.5 rounded-xl"
              >
            ...
            <View className="flex-row items-center gap-2">
              <View className="bg-[#1e40af] px-2.5 py-1 rounded-lg">
                <Text className="text-[11px] font-bold text-white">2D View</Text>
              </View>
            </View>
          </View>
            ...
            {loading ? (
              ...
              <View className="py-12 items-center">
                <Text className="text-xs font-semibold text-[#64748b]">
                  Không có dữ liệu khoang chứa cho kệ này trên máy chủ
                </Text>
              </View>
            ) : (
                              ...
                              >
                                ...
                                <Text
                                  className={`text-[11px] font-semibold mt-1.5 ${
                                    isSuggested ? 'text-[#15803d]' : 'text-[#64748b]'
                                  }`}
                                >
                                ...
                                <Text className="text-[10px] text-[#94a3b8] mt-0.5">
                                  Còn vừa {cell.usableCapacityPackages ?? 72} thùng
                                </Text>
                              </TouchableOpacity>
                            );
                          })}
            ...
            {activeCell ? (
              <View className="mt-4 bg-white p-4 rounded-2xl border border-[#e2e8f0] shadow-sm">
                  ...
                  <View className="bg-[#f0fdf4] p-3 rounded-xl border border-[#bbf7d0] mt-3 flex-row items-center gap-2">
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="text-xs text-[#166534]">
                        Khoang còn nhận tối đa {activeCell.usableCapacityPackages} thùng theo thể tích.
                      </Text>
                    </View>
                  </View>
                ) : null}
                ...
                <View className="bg-[#f8fafc] p-3.5 rounded-xl border border-[#e2e8f0] mt-3">
                  {itemName ? (
                    <Text className="text-sm font-extrabold text-[#1e293b]">{itemName}</Text>
                  ) : null}

                  {sku ? (
                    <Text className="text-xs font-bold text-[#0878f9] mt-0.5">SKU: {sku}</Text>
                  ) : null}

                  <Text className="text-xs font-extrabold text-[#0f172a] mt-1.5">
                    {activeCell.packageCount > 0
                      ? `Số lượng: ${activeCell.packageCount} thùng`
                      : 'Khoang hiện tại đang trống (0 thùng)'}
                ...
                {!readOnly ? (
                  <TouchableOpacity
                    ...
                    className="bg-[#1d4ed8] py-3.5 rounded-xl items-center justify-center mt-3 shadow-sm"
                  >
